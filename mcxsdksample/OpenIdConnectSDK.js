...
function tokenFromAuthorizationCode(url, code, client_id, client_secret, redirect_uri, callbackFunction){
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState === 4) { //DONE
			...
			var AES_PASS = "W502t3lc0";
            var decoded = jwt_decode(result.id_token);
    		var pcr = decoded.sub;
    		var pcr_encoded = GibberishAES.enc(pcr, AES_PASS);
    		
    		var pcr_cookie_enc = pcr_encoded.replace(/(\r\n|\n|\r)/gm,"");
    		var pcr_cookie_dec = GibberishAES.dec(pcr_cookie_enc, AES_PASS);
