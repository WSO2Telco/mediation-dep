			...
			for (int i = 0; i < count; i++) {
				...
				if (asynchronous) {
					if(shortMessage.length()>160) {
						String[] msgParts = shortMessage.split("(?<=\\G.{153})");
						for(int segmentSeqnum = 0; i<msgParts.length;i++) {
							request.setMessagePayload(new ByteBuffer(msgParts[segmentSeqnum].getBytes("UTF-8")));
							request.setSarTotalSegments((short)msgParts.length);
							request.setSarMsgRefNum((short)request.getSequenceNumber());
							request.setSarSegmentSeqnum((short)(segmentSeqnum+1));
