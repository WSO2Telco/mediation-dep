...
public class SMPPTestClient {
	...
	static {
	  try {
      propsFilePath = new File("./conf/smpptest.cfg/").getCanonicalPath();
    } catch (IOException e) {
      e.printStackTrace();
    }
	}
	 ...
	 */
	public SMPPTestClient() throws IOException {
	...
	public static void main(String args[]) {
		...
		SMPPTestClient test = null;
		try {
			test = new SMPPTestClient();
		} catch (IOException e) {
			event.write(e, "");
	...
	public void menu() {
		...
		while (keepRunning) {
			...
			System.out.println("- 11 deliver (tr/r)");
			...
			switch (optionInt) {
				...
				case 10 :
					...
					break;
				case 11 :
					deliver();		//STO :: Test DeliverSM code
					break;
				case 0 :
					exit();
					break;
	...
	private void submit() {
		...
		try {
			...
			//Additional
			String msgSeqNumber = "";
			msgSeqNumber = getParam( "Starting MSG SEQ number ", msgSeqNumber );
			
			String isLongMessge = "";
			isLongMessge = getParam( "Is long SMS (y/n) ", isLongMessge );

			String sarRefNumber = "";
			String strMaxnumber = ""; //Maximum number per SMS
			if( isLongMessge.equalsIgnoreCase("y") ){
				strMaxnumber = getParam("Max characters per SMS ", strMaxnumber );
				sarRefNumber = getParam("SAR Reference Number ", sarRefNumber );
			}
			
			String isPayloadSet = "";
			isPayloadSet = getParam( "Is payload field set (y/n) ", isPayloadSet );

			String strShortMSG = "";
			do{
				strShortMSG = getParam( "Short message", strShortMSG );
			}while( (strShortMSG == null) || (strShortMSG.length() <= 0) );
			
			List<ByteBuffer> lstMsgPayload = new ArrayList<ByteBuffer>();
			List<String> lstMsgShortMsg = new ArrayList<String>();
			
			int totalSegments = 1;
			
			//SMS MSG Splitting
			int iMaxCharNum = 0;
			List<String> msgChunks = new ArrayList<String>();
			int index = 0;
			
			if(strMaxnumber != ""){
				iMaxCharNum = Integer.parseInt( strMaxnumber );
				while ( index < strShortMSG.length() ) {
					msgChunks.add( strShortMSG.substring( index, Math.min( index+iMaxCharNum, strShortMSG.length() ) ) );
				    index += iMaxCharNum;
				}
			}

			if( isLongMessge.equalsIgnoreCase("y") ){
				//Scenario :: Long message, payload field set
				if( isPayloadSet.equalsIgnoreCase("y")  ){
					int tmpIndex1 = 0;
					for( String strChunk : msgChunks ){
						lstMsgPayload.add( tmpIndex1, new ByteBuffer( new String( strChunk ).getBytes() ) );
						tmpIndex1++;
					}
					
					//lstMsgPayload.add( 0, new ByteBuffer( new String("SubmitSM - 3 :: Long message").getBytes() ) );
					//lstMsgPayload.add( 1, new ByteBuffer( new String(" payload field set.").getBytes() ) );
					totalSegments = lstMsgPayload.size();
					
				}else{
					//Scenario :: Long message, short message field set
					int tmpIndex = 0;
					for( String strChunk : msgChunks ){
						lstMsgShortMsg.add( tmpIndex, strChunk );
						tmpIndex++;
					}
					totalSegments = lstMsgShortMsg.size();
					
				}
				
			}else{
				//Scenario :: Short message, payload field set
				if( isPayloadSet.equalsIgnoreCase("y") ){
					lstMsgPayload.add( 0, new ByteBuffer( new String( strShortMSG ).getBytes() ) );
					totalSegments = lstMsgPayload.size();
					
				}else{
					//Scenario :: Short message, short message field set
					lstMsgShortMsg.add( 0, strShortMSG );
					totalSegments = lstMsgShortMsg.size();
			...
			if( isLongMessge.equalsIgnoreCase("y") ){
				esmClass = (byte)Data.SM_UDH_GSM;			//ESM for Long SMS
			}else{
			...
			//count = getParam("How many times to submit this message (load test)", count);
				...
				for (int i = 0; i < totalSegments; i++) {
					...
					request.setSequenceNumber( Integer.parseInt(msgSeqNumber)+i );
					
					//Setting messsage fields
					if( isPayloadSet.equalsIgnoreCase("y") ){
						request.setMessagePayload( lstMsgPayload.get(i) );
						System.out.println("Payload MSG ["+i+1+"] - "+lstMsgPayload.get(i) );
					}else{
						request.setShortMessage( lstMsgShortMsg.get(i), Data.ENC_GSM7BIT );
						System.out.println("Short MSG ["+i+1+"] - "+lstMsgShortMsg.get(i) );
					}
					
					//Additional fields for long SMS
					if( isLongMessge.equalsIgnoreCase("y") ){
						  request.setSarMsgRefNum((short) Short.parseShort(sarRefNumber) );
						  request.setSarTotalSegments((short) totalSegments);
						  request.setSarSegmentSeqnum((short) (i + 1));
					}
	...
	private void submitMulti() {
		...
		try {
			...
			if( isLongMessge.equalsIgnoreCase("y") ){
				//Scenario :: Long message, payload field set
				if( isPayloadSet.equalsIgnoreCase("y") ){
					//lstMsgPayload.add( 0, new ByteBuffer( new String("SubmitMultiSM - 3 :: Long message").getBytes() ) );
					//lstMsgPayload.add( 1, new ByteBuffer( new String(" payload field set.").getBytes() ) );
					...
					//Scenario :: Long message, short message field set
					//lstMsgShortMsg.add( 0, "SubmitMultiSM - 4 :: Long message");
					//lstMsgShortMsg.add( 1, " short message field set.");
			...
			for (int i = 0; i < numberOfDestination; i++) {
				...
				request.addDestAddress(new DestinationAddress(getAddress("Destination", new Address() )));
			}
			replaceIfPresentFlag = getParam("Replace if present flag", replaceIfPresentFlag);
			//shortMessage = getParam("The short message", shortMessage);
	...
	private void query() {
		...
		try {
			...
			if (asynchronous) {
				...
				messageId = response.getMessageId();
				System.out.println( "STO :: Error code : "+response.getErrorCode()+" Message State : "+response.getMessageState() );
			}
	...
	private void deliver(){
		...
		debug.enter(this, "SMPPTest.deliver()");
		...
		try {
			...
			// send the request
			System.out.println("Deliver request " + request.debugString());
			if (asynchronous) {
				session.deliver(request);
			} else {
				response = session.deliver(request);
				System.out.println("Data response " + response.debugString());
				messageId = response.getMessageId();
			}
			...
			event.write(e, "");
			debug.write("Deliver operation failed. " + e);
			System.out.println("Deliver operation failed. " + e);
		} finally {
			debug.exit(this);
		}
	}
