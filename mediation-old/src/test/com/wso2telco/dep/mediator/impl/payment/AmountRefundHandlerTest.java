...
public class AmountRefundHandlerTest {
    @Test
    public void testNullOrTrimmed_WhenStringIsNull() {
        String s = AmountRefundHandler.nullOrTrimmed(null);
        Assert.assertEquals(s, null);
    }

    @Test
    public void amountRefundHandler_test() throws Exception {
        ...
        JSONObject jsonObject = new JSONObject("{\"amountTransaction\":{\"endUserId\":\"tel:+SBcRDj/+M108gFCu1S56zw==\"," +
                "\"transactionOperationStatus\":\"Refund\",\"clientCorrelator\":\"TES35cctrd25\",\"referenceCode\":\"REF-TEce2dfdwe\"," +
        ...
        AmountRefundHandler amountRefundHandler = new AmountRefundHandler(mockOcci, mockPaymentService, mockPaymentExecutor,
                mockApiUtils, mockPaymentUtil, mockCarbonUtilsDelegator, mockValidatorUtilsDelegator, mockFileReader);
        boolean result = amountRefundHandler.handle(mockAxis2MessageContext);
        Assert.assertEquals(result, true);
    }

    @Test(expectedExceptions = NullPointerException.class)
    public void amountRefundHandler_whenOparatorEndPointSearchDTOisNull() throws Exception {
        MessageContext mockMessageContext = Mockito.mock(MessageContext.class);
        Axis2MessageContext mockAxis2MessageContext = Mockito.mock(Axis2MessageContext.class);
        Mockito.when(mockAxis2MessageContext.getAxis2MessageContext()).thenReturn(mockMessageContext);
        HashMap headerMap = new HashMap<>();
    ...
    @Test
    public void amountRefundHandler_whenUserAnonymization() throws Exception {
        MessageContext mockMessageContext = Mockito.mock(MessageContext.class);
        Axis2MessageContext mockAxis2MessageContext = Mockito.mock(Axis2MessageContext.class);
        Mockito.when(mockAxis2MessageContext.getAxis2MessageContext()).thenReturn(mockMessageContext);
        HashMap headerMap = new HashMap<>();
